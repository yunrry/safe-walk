...
class HolidayAccidentImporter:
    ...
    def create_table(self):
        """데이터베이스 테이블 생성"""
        try:
            print("🏗️ 휴일 사고 다발지 테이블 생성 중...")
            
            create_table_sql = """
            CREATE TABLE IF NOT EXISTS holiday_accident_hotspots (
            ...
            ) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci COMMENT='휴일 사고 다발지 데이터';
            """
    ...
    def add_legal_dong_column(self):
        ...
        try:
            ...
            AND TABLE_NAME = 'holiday_accident_hotspots' 
            ...
            with self.engine.connect() as connection:
                ...
                if not column_exists:
                    ...
                    add_column_sql = """
                    ALTER TABLE holiday_accident_hotspots 
    ...
    def save_to_database(self, df):
        ...
        try:
            ...
            print("🗑️ 기존 데이터 삭제 중...")
            with self.engine.connect() as connection:
                connection.execute(text("DELETE FROM holiday_accident_hotspots"))
                connection.commit()
            print("✅ 기존 데이터 삭제 완료")
            
            # 테이블에 데이터 삽입
            df.to_sql('holiday_accident_hotspots', 
                     con=self.engine, 
                     if_exists='append', 
                     index=False, 
                     method='multi',
            ...
            # 저장된 데이터 확인
            with self.engine.connect() as connection:
                result = connection.execute(text("SELECT COUNT(*) as total FROM holiday_accident_hotspots"))
...
def main():
    ...
    try:
        # CSV 파일 경로
        csv_path = "csv/HolidayAccident.csv"
        
        # 임포터 인스턴스 생성 및 실행
        importer = HolidayAccidentImporter()
