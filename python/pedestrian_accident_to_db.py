...
class PedestrianAccidentImporter:
    def __init__(self):
        """초기화 및 환경변수 로드"""
        load_dotenv()
    ...
    def create_table(self):
        """데이터베이스 테이블 생성"""
        try:
            print("🏗️ 보행자 사고 다발지 테이블 생성 중...")
            
            create_table_sql = """
            CREATE TABLE IF NOT EXISTS pedestrian_accident_hotspots (
            ...
            ) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci COMMENT='보행자 사고 다발지 데이터';
            """
    ...
    def add_legal_dong_column(self):
        ...
        try:
            ...
            AND TABLE_NAME = 'pedestrian_accident_hotspots' 
            ...
            with self.engine.connect() as connection:
                ...
                if not column_exists:
                    ...
                    add_column_sql = """
                    ALTER TABLE pedestrian_accident_hotspots 
    ...
    def save_to_database(self, df):
        ...
        try:
            ...
            print("🗑️ 기존 데이터 삭제 중...")
            with self.engine.connect() as connection:
                connection.execute(text("DELETE FROM pedestrian_accident_hotspots"))
                connection.commit()
            print("✅ 기존 데이터 삭제 완료")
            
            # 테이블에 데이터 삽입
            df.to_sql('pedestrian_accident_hotspots', 
                     con=self.engine, 
                     if_exists='append', 
                     index=False, 
                     method='multi',
            ...
            # 저장된 데이터 확인
            with self.engine.connect() as connection:
                result = connection.execute(text("SELECT COUNT(*) as total FROM pedestrian_accident_hotspots"))
...
def main():
    ...
    try:
        ...
        # 임포터 인스턴스 생성 및 실행
        importer = PedestrianAccidentImporter()
